...
const modes: { value: ViewMode; icon: typeof Columns2; label: string }[] = [
  { value: 'both', icon: Rows2, label: 'Stacked' },
  { value: 'side-by-side', icon: Columns2, label: 'Side by side' },
    ...
    <div className="flex h-8 items-center rounded-lg border border-divider bg-sidebar dark:border-divider-dark dark:bg-bg-dark">
      {modes.map(({ value, icon: Icon, label }) => (
        <button
          ...
          className={[
            'flex h-full cursor-pointer items-center gap-1.5 px-2.5 text-xs transition-colors first:rounded-l-lg last:rounded-r-lg',
            viewMode === value
